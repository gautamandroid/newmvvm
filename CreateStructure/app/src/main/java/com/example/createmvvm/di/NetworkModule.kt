...
import com.example.createmvvm.api.AppApi
...
class NetworkModule {
    ...
    fun provideAPIService(): AppApi {
        ...
        httpClient.addInterceptor { chain ->
            ...
            val requestBuilder = original.newBuilder().addHeader("Content-Type", "application/json").addHeader("Cache-Control", "no-cache")
            // if (!token.isNullOrEmpty()) {
            //      requestBuilder.addHeader("Authorization", "Bearer $token")
            //  }
            //   Log.e("TAG", "provideAPIService: " + token)
